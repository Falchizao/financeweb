...
public interface MovimentationRepository extends IRepository<Movimentation> {
    ...
    @Query("SELECT obj FROM tb_movimentation obj INNER JOIN tb_account acc on obj.account.id = acc.id INNER JOIN tb_user us on acc.user.id = us.id where us.username =:username" +
    ...
    @Transactional
    @Modifying
    @Query("Update tb_movimentation mv set mv.paymentDate=:paidDate where mv.id=:idMov")
    int setAsPaid(Long idMov, LocalDate paidDate);
}
